...
export const adminReactivateProjectSchema = {
  project_id: z
    .string()
    .describe(
      "The project ID to un-archive. Platform-admin only — flips `projects.archived_at` back to NULL. In v1.57 this was narrowed: it does NOT reactivate a grace-state account. For that, subscribe a tier (`run402 tier set <tier>`) or toggle the account-level escape hatch via `admin_set_lease_perpetual`.",
    ),
};
